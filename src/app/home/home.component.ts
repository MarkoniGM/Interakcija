...
export class HomeComponent {
  ...
  public doAddToCart(id: number) {
    AlertService.question('Add to cart', `Do you want to add part ${id} to cart?`)
      .then(rsp => {
        if (rsp.isConfirmed) {
          if (!this.userService.hasActive()) {
            AlertService.error('You have to be signed in', 'You cant add parts to the cart if you are not signed in!')
